...
resolve_env_file() {
  ...
  if [[ "${mode}" == "development" ]]; then
    ...
    echo "No se encontro ${env_file} ni .env.development.example ni .env.example" >&2
    exit 1
  fi
  ...
  if [[ -f "${APP_DIR}/.env.example" ]]; then
    ...
    echo "Se creo ${APP_DIR}/.env desde .env.example. Ajusta valores antes de exponer."
    printf '%s\n' "${APP_DIR}/.env"
    return 0
  fi
...
deploy_frontend() {
  ...
  echo "Levantando TecnoLTS en ${mode} usando ${env_file}..."
  ...
  if [[ "${mode}" == "development" ]]; then
    ...
    if [[ "${dev_runtime}" == "stable" ]]; then
      ...
      compose_cmd "${env_file}" "${mode}" run --rm --no-deps web-dev \
