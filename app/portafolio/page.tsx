...
import PortafolioContent from './_components/PortafolioContent';
...
function ErrorPage({ message }: { message: string }) {
  return (
    <div className="flex min-h-screen items-center justify-center p-4 bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
      <div className="w-full max-w-[420px] rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-xl">
        <div className="mx-auto mb-6 h-1.5 w-12 rounded-full bg-gray-300" />
        <svg viewBox="0 0 94.33 22.04" className="mx-auto mb-5 h-5 opacity-40" aria-label="TecnoLTS">
          ...
          <path fill="#126e95" d="M51.82,17.94c-1-.56-1.79-1.33-2.38-2.33-.59-1-.88-2.12-.88-3.36s.29-2.36.88-3.35c.59-.99,1.38-1.76,2.38-2.32,1-.56,2.09-.83,3.29-.83s2.29.28,3.29.83c1,.56,1.78,1.33,2.36,2.32.57.99.86,2.11.86,3.35s-.29,2.37-.87,3.36c-.58,1-1.37,1.77-2.36,2.33-.99.56-2.09.83-3.28.83s-2.29-.28-3.29-.83Z"/>
...
export default async function PortafolioPage({
  ...
  if (sessionCookie?.value) {
    const sessionId = parseCookieValue(sessionCookie.value);
    if (sessionId) {
      ...
      if (session) {
        ...
        const sessionToken = store.tokens.find((t) => t.id === session.tokenId);
        if (sessionToken && sessionToken.token === tokenParam && sessionToken.active) {
          return <PortafolioContent />;
        }
      }
    }
  ...
  if (!tokenParam) {
    return <ErrorPage message="Enlace de acceso inválido o no proporcionado." />;
  }

  const tokenEntry = store.tokens.find(
    (t) => t.token === tokenParam && t.active
  );
  ...
  if (activeSession) {
    redirect(`/api/portafolio/session?token=${encodeURIComponent(tokenParam)}`);
  }
  ...
  try {
    ...
    return <ErrorPage message="Error al enviar el código de verificación. Intenta de nuevo." />;
  }
